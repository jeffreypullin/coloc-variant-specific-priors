...
```{r}
pqtl_metadata <- read_tsv("data/metadata/SomaLogic_Ensembl_96_phenotype_metadata.tsv.gz")
pqtl_metadata |>
  select(gene_start, gene_end, strand, phenotype_pos)
```
...
pqtl_eqtl_colocs |>
  mutate(
    called_coloc_unif = PP.H4.abf_unif >= 0.8,
    called_coloc_non_unif = PP.H4.abf_tss_non_unif >= 0.8
  ) |>
  ...
  select(PP.H4.abf_unif, PP.H4.abf_tss_non_unif, gene_name, protein_name)
...
pqtl_eqtl_colocs |>
  mutate(
    called_coloc_unif = PP.H4.abf_unif >= 0.8,
    called_coloc_non_unif = PP.H4.abf_score_non_unif >= 0.8
  ) |>
  count(called_coloc_unif, called_coloc_non_unif)
