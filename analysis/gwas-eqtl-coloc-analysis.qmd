...
gwas_eqtl_coloc_abf_data <- expand_grid(
  chr = 1:22,
  ...
  mutate(file = here::here(
    glue("data/output/gwas-eqtl-coloc-abf-{gwas_id}-{eqtl_id}-{chr}.rds")
  )) |>
  mutate(data = list(read_rds(file))) |>
  unnest(data)

gwas_eqtl_coloc_susie_data <- expand_grid(
  chr = 1:22,
  gwas_id_eqtl_id = gwas_ids_eqtl_ids
  ) |>
  separate_wider_delim(gwas_id_eqtl_id, "-", names = c("gwas_id", "eqtl_id")) |>
  rowwise() |>
  mutate(file = here::here(
    glue("data/output/gwas-eqtl-coloc-susie-{gwas_id}-{eqtl_id}-{chr}.rds")
  )) |>
  mutate(data = list(read_rds(file))) |>
  unnest(data)
```
...
gwas_eqtl_coloc_susie_data |>
  mutate(
    sig_abc_score_gwas = PP.H4.abf_eqtl_tss_eqtlgen > 0.8,
    sig_unif = PP.H4.abf_unif > 0.8
  ) |>
  arrange(gwas_id) |>
  filter(sig_abc_score_gwas != sig_unif) |>
...
VAV3 `PP.H4.abf_eqtl_tss_eqtlgen` success!!
ACADVL `PP.H4.abf_eqtl_tss_eqtlgen` also a success!!
PSMB7 `PP.H4.abf_eqtl_tss_eqtlgen` a success!!

```{r}
gwas_eqtl_coloc_abf_data |>
  mutate(
    sig_abc_score_gwas = PP.H4.abf_gwas_tss_eqtlgen > 0.8,
    sig_unif = PP.H4.abf_unif > 0.8
  ) |>
  rowwise() |>
  mutate(
...
gwas_eqtl_colocs |>
  mutate(
    sig_gwas_tss_eqtlgen = PP.H4.abf_abc_score_eqtl > 0.5,
    sig_unif = PP.H4.abf_unif > 0.5
  ) |>
  arrange(gwas_id) |>
  filter(sig_gwas_tss_eqtlgen != sig_unif) |>
  select(gwas_id, eqtl_id, gene_name,
