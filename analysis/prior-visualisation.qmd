...
dists <- seq(1, 1.2e6, by = 1e4)
prop_lt <- map_dbl(dists, function(x) {
  sum(abs_tss_distance < x) / length(abs_tss_distance)
})
...
tibble(
  ...
  ggplot(aes(dists, prop_lt)) + 
  geom_point() +  
  geom_vline(xintercept = 1e5, linetype = "dotted") + 
  geom_vline(xintercept = 5e5, linetype = "dotted") + 
  geom_vline(xintercept = 1e6, linetype = "dotted") + 
  labs(
    y = "Proportion of eQTLs within +/- distance of TSS",
    x = "Distance (bp)"
  ) +  
  theme_bw()
```
...
abc_score_data <- read_tsv(here::here("data/abc-data.txt.gz"), show_col_types = FALSE)
