...
gene_metadata <- read_tsv(
  here::here("data/metadata/gene_counts_Ensembl_105_phenotype_metadata.tsv.gz"),
  show_col_types = FALSE)
)
```
...
pqtl_eqtl_coloc_abf_data |>
  left_join(
    protein_metadata |>
      ...
      select(phenotype_id, protein_tss),
    by = "phenotype_id"
  ) |>
  left_join(
    gene_metadata |>
      mutate(gene_tss = if_else(strand == 1, gene_start, gene_end)) |>
      select(gene_id, gene_tss),
    by = "gene_id"
  ) |>
  filter(PP.H4.abf_unif > 0.5) |>
  select(gene_tss, protein_tss, PP.H4.abf_unif, 
         PP.H4.abf_eqtl_tss_eqtlgen, phenotype_id, 
         gene_name, eqtl_data_id) |>
  mutate(
    pos_diff = log10(abs(gene_tss - protein_tss) + 1),
    h4_diff = PP.H4.abf_eqtl_tss_eqtlgen - PP.H4.abf_unif 
  ) |>
  ggplot(aes(pos_diff, h4_diff)) + 
  geom_point()
```
...
pqtl_eqtl_coloc_abf_data |>
  ...
  filter(pos_diff == 0 & h4_diff < -0.1) |>
  select(phenotype_id, gene_name, 
         PP.H4.abf_eqtl_tss_eqtlgen, PP.H4.abf_unif)
```


```{r}
pqtl_eqtl_coloc_abf_data |>
  filter(nsnps_unif <= 5000) |> 
  ggplot(aes(nsnps_unif, PP.H4.abf_unif)) + 
  geom_point() + 
...
pqtl_eqtl_coloc_abf_data |>
  mutate(
    ...
    sig_eqtlgen = PP.H4.abf_abc_score_eqtl > 0.8
  ) |>
  filter(sig_unif != sig_eqtlgen) |>
  dplyr::select(eqtl_data_id, gene_name, phenotype_id, 
...
weights <- list()
for (i in 1:500) {
  weight <- compute_rand_prior_weights(test[[1]]$position)
  weights[[i]] <- weight
  h4[[i]] <- coloc.abf(
    test[[1]],
    test[[2]],
    prior_weights1 = weight
  )$summary[["PP.H4.abf"]]
}
hist(h4)

Y <- as.matrix(h4)
X <- do.call(rbind, weights)
cv <- cv.glmnet(x = X, y = Y, alpha = 1) 
cv
lasso <- glmnet(x = X, y = Y, alpha = 1, lambda = 0.005119)
as.numeric(lasso$beta)

plot(test[[1]]$position, as.numeric(lasso$beta))
which.min(as.numeric(lasso$beta))


eqtlgen_density_data <- read_rds("output/densities/eqtlgen.rds")
eqtl_prior_weights_eqtlgen <- compute_eqtl_tss_dist_prior_weights(
  test[[1]]$position, 177333822, eqtlgen_density_data
)
pqtl_prior_weights_eqtlgen <- compute_eqtl_tss_dist_prior_weights(
  test[[2]]$position, 177351840, eqtlgen_density_data
)
...
coloc_level_coloc_abf_data <- left_join(
  pqtl_eqtl_coloc_abf_data |>
    select(eqtl_data_id, starts_with("PP.H4.abf"), phenotype_id, 
           coloc_gene_id = gene_id, gene_name), 
  protein_metadata |>
...
pqtl_eqtl_coloc_abf_data |>
  filter(gene_name == "") |>
  select(eqtl_data_id, gene_name, phenotype_id, PP.H4.abf_unif)
...
```{r}
coloc_susie_perf_data |>
  filter(eqtl_data_id %in% c("QTD000373", "QTD000116", "QTD000341")) |>
  summarise(
    recall = median(recall),
    precision = median(precision),
    .by = prior_type
