...
eqtl_data <- tabix.read.table("data/eqtl-catalogue/sumstats/QTD000373.cc.tsv.gz", 
                              paste0(13, ":1-2147483647")) |>
  as_tibble() |>
  setNames(eqtl_col_names) |>
  filter(molecular_trait_id == "ENSG00000102805") |>
...
pqtl_data <- tabix.read.table("data/QTD000584.cc.tsv.gz", paste0(13, ":1-2147483647")) |>
  as_tibble() |>
  # All the column names are the same.
  setNames(eqtl_col_names) |>
  filter(molecular_trait_id == "CLN5.8874.53.3..1") |>
  filter(chromosome == 13) |>
  filter(
    position >= 76890660 &
    position <= 77090660
...
suppressWarnings({
  coloc.abf(eqtl_dataset, pqtl_dataset)$summary
  coloc.abf(eqtl_dataset, pqtl_dataset, tss1 = 520391)$summary
  coloc.abf(eqtl_dataset, pqtl_dataset, score_data1 = gnocchi_data, chrom = 1)$summary
})

coloc_score_out <- coloc.abf(eqtl_dataset, pqtl_dataset, 
                             score_data1 = gnocchi_data, chrom = 1)

tibble(pos = eqtl_data$position, p = coloc_score_out$priors$p1_vec) |>
  ggplot(aes(pos, p)) + 
  geom_point()

plot(coloc_score_out$priors$p1_vec)
```
