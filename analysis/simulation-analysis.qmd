...
sim_loci <- config$simulation_loci
simulation_data <- lapply(sim_loci, function(x) {
...
pos <- list()
locus <- list()
for (i in seq_along(sim_loci)) {
  file <- glue(here::here("data/{sim_loci[[i]]}.vcf.gz.impute.legend"))
  ...
  locus[[i]] <- rep(sim_loci[[i]], length(pos[[i]]))
}
